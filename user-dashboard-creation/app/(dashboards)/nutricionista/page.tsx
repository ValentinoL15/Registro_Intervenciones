...
import { Apple, Trash2, History, Loader2, PlusCircle, FileText, Edit, Eye, ZoomIn, ExternalLink } from "lucide-react";
...
export default function NutricionistaPage() {
                ...
                <TableBody>
                  {currentItems.map((item) => (
                    <TableRow key={item.id}>
                      <TableCell className="font-medium">{item.fechaInicio} al {item.fechaFinal}</TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
  ...
  fecha: `${item.fechaInicio} al ${item.fechaFinal}`
                      ...
                      <TableCell className="text-right flex justify-end gap-1">
                        <Button variant="ghost" size="icon" className="text-blue-600" onClick={() => setEditingItem(item)}><Edit className="w-5 h-5" /></Button>
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => handleDelete(item.id)} className="bg-destructive">Sí, borrar</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </TableCell>
